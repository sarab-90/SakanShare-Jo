...
export const preferencesSchema = Joi.object({
  gender: Joi.string().valid("male", "female", "any").messages({
    "any.only": "Gender must be one of: male, female, or any",
  }),

  min_age: Joi.number().integer().min(17).max(100).messages({
    "number.base": "Minimum age must be a number",
    "number.min": "Minimum age must be at least 17",
  }),

  max_age: Joi.number().integer().min(Joi.ref("min_age")).max(100).messages({
  ...
  smoking: Joi.boolean().messages({
    "boolean.base": "Smoking field must be true or false",
  }),

  sleep_time: Joi.string().max(50).allow(null, "").messages({
    "string.max": "Sleep time description is too long (max 50 chars)",
  }),

  cleanliness: Joi.string().max(50).allow(null, "").messages({
  ...
  pets_allowed: Joi.boolean().allow(null).messages({
    "boolean.base": "Pets allowed field must be true or false",
  }),

  guest_policy: Joi.string().max(50).allow(null, "").messages({
    "string.max": "Guest policy description is too long (max 50 chars)",
  }),

  additional_notes: Joi.string().max(500).allow(null, "").messages({
    "string.max": "Additional notes cannot exceed 500 characters",
  }),

  budget: Joi.number().precision(2).positive().min(0).max(1000000).messages({
  ...
  city: Joi.string().max(50).allow(null, "").messages({
    "string.max": "City name is too long (max 50 chars)",
  }),
  preferred_room_type: Joi.string()
    .valid("private", "shared", "studio")
    .allow(null, ""),
